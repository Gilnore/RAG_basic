...
class mem:
    ...
    def temp_mem(self, text:str, force_recreate = False, embed_instruction: str = 'Represent the document for retrieval: '):
        ...
        try:
            vector_store.aadd_documents(docs)
        except ValueError:
    ...
    def query(self, query:str,query_instruction: str = 'Represent the question for retrieving supporting documents: '):
...
def main():
    with open('langchain.txt','r') as doc:
        ...
        vec = mem(local = False)
        # vec.temp_mem(text)
        vec.perm_mem(text, force_recreate=True,metric = Distance.COSINE)
        return vec
